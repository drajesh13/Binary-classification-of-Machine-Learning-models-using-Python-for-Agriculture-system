...
class XGBooster:
    ...
    def predict(self, X):
        ...
        print("all preds",all_tree_preds)
        return np.round(self.sigmoid(all_tree_preds)).astype(int)  # Round to nearest integer for class labels
...
class DecisionTree:
    ...
    def fit(self, X, y, depth=0):
        ...
        if len(unique_classes) == 1 or depth == self.max_depth:
            self.tree = {'class': unique_classes[0], 'is_leaf': True}
            return
        ...
        self.tree = {'feature_index': best_split['feature_index'], 'value': best_split['value'], 'is_leaf': False}
